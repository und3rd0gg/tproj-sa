...
namespace FlatKit
{
    ...
    public class ReadmeEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Documentation")) OpenDocumentation();
...

            {
                ...
                if (GUILayout.Button("Open support ticket")) OpenSupportTicket();

                if (GUILayout.Button("Copy debug info")) CopyDebugInfoToClipboard();
...

            {
                if (!_readme.FlatKitInstalled)
                {
                    ...
                    if (GUILayout.Button("URP")) UnpackFlatKitUrp();

                    if (GUILayout.Button("Built-in RP")) UnpackFlatKitBuiltInRP();
...

            {
                if (!string.IsNullOrEmpty(_readme.PackageManagerError))
                {
                    EditorGUILayout.Separator();
                    DrawUILine(Color.yellow, 1, 0);
                    EditorGUILayout.HelpBox($"Package Manager error: {_readme.PackageManagerError}",
                        MessageType.Warning);
                    DrawUILine(Color.yellow, 1, 0);
                }
            }

            {
                ...
                if (GUILayout.Button("URP", EditorStyles.miniButtonLeft)) ConfigureUrp();

                if (GUILayout.Button("Built-in RP", EditorStyles.miniButtonLeft)) ConfigureBuiltIn();
...

            {
                ...
                if (GUILayout.Button("Clear cache")) ClearPackageCache();

                if (GUILayout.Button($"Select {AssetName}")) OpenPackageManager();
        ...
        private void ClearPackageCache()
        {
            ...
            if (Application.platform == RuntimePlatform.OSXEditor) path = "~/Library/Unity/Asset Store-5.x/Dustyroom/";
        ...
        {
            var path = AssetDatabase.GUIDToAssetPath(UnityPackageUrpGuid.ToString());
            if (path == null)
        ...
        {
            var path = AssetDatabase.GUIDToAssetPath(UnityPackageBuiltInGuid.ToString());
            if (path == null)
        ...
        {
            var path = AssetDatabase.GUIDToAssetPath(UrpPipelineAssetGuid.ToString());
            if (path == null)
            {
