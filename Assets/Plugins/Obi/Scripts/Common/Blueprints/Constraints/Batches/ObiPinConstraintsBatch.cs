...
namespace Obi
{
    ...
    public class ObiPinConstraintsBatch : ObiConstraintsBatch
    {
        ...
        public void AddConstraint(int solverIndex, ObiColliderBase body, Vector3 offset, Quaternion restDarboux,
            float linearCompliance, float rotationalCompliance, float breakThreshold)
        {
            RegisterConstraint();
        ...
        public override void Merge(ObiActor actor, IObiConstraintsBatch other)
        {
            ...
            if (batch != null)
            {
                ...
                restDarbouxVectors.CopyFrom(batch.restDarbouxVectors, 0, m_ActiveConstraintCount,
                    batch.activeConstraintCount);
                stiffnesses.CopyFrom(batch.stiffnesses, 0, m_ActiveConstraintCount * 2,
                    batch.activeConstraintCount * 2);
                breakThresholds.CopyFrom(batch.breakThresholds, 0, m_ActiveConstraintCount,
                    batch.activeConstraintCount);

                for (var i = 0; i < batch.activeConstraintCount; ++i)
                {
                    particleIndices[m_ActiveConstraintCount + i] = batch.particleIndices[i];
                    colliderIndices[m_ActiveConstraintCount + i] =
                        batch.pinBodies[i] != null ? batch.pinBodies[i].index : -1;
                }
        ...
        public override void AddToSolver(ObiSolver solver)
        {
            if (solver != null && solver.implementation != null)
            {
                ...
                if (m_BatchImpl != null)
                    m_BatchImpl.SetPinConstraints(particleIndices, colliderIndices, offsets, restDarbouxVectors,
                        stiffnesses, lambdas, m_ActiveConstraintCount);
            }
        }
