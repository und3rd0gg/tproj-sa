    ...
    public class ObiVolumeConstraintsBatch : ObiConstraintsBatch
    {
        ...
        public override void Merge(ObiActor actor, IObiConstraintsBatch other)
        {
            ...
            if (batch != null && user != null)
            {
                ...
                pressureStiffness.CopyReplicate(new Vector2(user.pressure, user.compressionCompliance),
                    m_ActiveConstraintCount, batch.activeConstraintCount);

                for (var i = 0; i < numActiveTriangles * 3; ++i)
                    particleIndices[initialIndexCount + i] = actor.solverIndices[batch.particleIndices[i]];

                for (var i = 0; i < batch.activeConstraintCount + 1; ++i)
        ...
        public override void AddToSolver(ObiSolver solver)
        {
            ...
            if (m_BatchImpl != null)
                m_BatchImpl.SetVolumeConstraints(particleIndices, firstTriangle, numTriangles, restVolumes,
                    pressureStiffness, lambdas, m_ActiveConstraintCount);
        }
        ...
        {
            var p = new Vector2(pressure, compliance);
            for (var i = 0; i < pressureStiffness.count; i++)
