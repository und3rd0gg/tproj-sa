...
namespace Obi
{
    ...
    public class ObiShapeMatchingConstraintsBatch : ObiConstraintsBatch
    {
        ...
        {
            var first = firstIndex[index];
            var num = numIndices[index];
        ...
        {
            var first = firstIndex[constraintIndex];
            var num = numIndices[constraintIndex];

            var found = 0;
            for (var i = first + num - 1; i >= first; --i)
            ...
            for (var i = constraintIndex + 1; i < constraintCount; ++i)
                firstIndex[i] -= found;
        }
        ...
        public override void Merge(ObiActor actor, IObiConstraintsBatch other)
        {
            ...
            if (batch != null && user != null)
            {
                ...
                for (var i = 0; i < batch.constraintCount; ++i)
                ...
                plasticDeformations.ResizeInitialized(m_ActiveConstraintCount + batch.activeConstraintCount,
                    Matrix4x4.identity);
                ...
                orientations.CopyReplicate(actor.actorLocalToSolverMatrix.rotation, m_ActiveConstraintCount,
                ...
                for (var i = 0; i < batch.activeConstraintCount; ++i)
                {
                    ...
                    materialParameters[(m_ActiveConstraintCount + i) * 5] =
                        batch.materialParameters[i * 5] * user.deformationResistance;
                    materialParameters[(m_ActiveConstraintCount + i) * 5 + 1] =
                        batch.materialParameters[i * 5 + 1] * user.plasticYield;
                    materialParameters[(m_ActiveConstraintCount + i) * 5 + 2] =
                        batch.materialParameters[i * 5 + 2] * user.plasticCreep;
                    materialParameters[(m_ActiveConstraintCount + i) * 5 + 3] =
                        batch.materialParameters[i * 5 + 3] * user.plasticRecovery;
                    materialParameters[(m_ActiveConstraintCount + i) * 5 + 4] =
                        batch.materialParameters[i * 5 + 4] * user.maxDeformation;
                }
        ...
        public override void AddToSolver(ObiSolver solver)
        {
            ...
            m_BatchImpl =
                solver.implementation.CreateConstraintsBatch(constraintType) as IShapeMatchingConstraintsBatchImpl;
