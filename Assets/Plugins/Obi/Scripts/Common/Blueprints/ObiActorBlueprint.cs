...
namespace Obi
{
    public abstract class ObiActorBlueprint : ScriptableObject, IObiParticleCollection
    {
        ...
        [HideInInspector] [SerializeField] protected int m_ActiveParticleCount;
        [HideInInspector] [SerializeField] protected int m_InitialActiveParticleCount;
        [HideInInspector] [SerializeField] protected Bounds _bounds;

        /**Particle components*/
        [HideInInspector] public Vector3[] positions;

        /**< Particle positions.*/
        [HideInInspector] public Vector4[] restPositions;

        /**< Particle rest positions, used to filter collisions.*/
        [HideInInspector] public Quaternion[] orientations;

        /**< Particle orientations.*/
        [HideInInspector] public Quaternion[] restOrientations;

        /**< Particle rest orientations.*/
        [HideInInspector] public Vector3[] velocities;

        /**< Particle velocities.*/
        [HideInInspector] public Vector3[] angularVelocities;
        ...
        [FormerlySerializedAs("phases")] [HideInInspector]
        public int[] filters;

        /**< Particle filters*/
        [HideInInspector] public Vector3[] principalRadii;
        ...
        [HideInInspector] public ObiDistanceConstraintsData distanceConstraintsData;

        [HideInInspector] public ObiBendConstraintsData bendConstraintsData;
        [HideInInspector] public ObiSkinConstraintsData skinConstraintsData;
        [HideInInspector] public ObiTetherConstraintsData tetherConstraintsData;
        [HideInInspector] public ObiStretchShearConstraintsData stretchShearConstraintsData;
        [HideInInspector] public ObiBendTwistConstraintsData bendTwistConstraintsData;
        [HideInInspector] public ObiShapeMatchingConstraintsData shapeMatchingConstraintsData;
        [HideInInspector] public ObiAerodynamicConstraintsData aerodynamicConstraintsData;
        [HideInInspector] public ObiChainConstraintsData chainConstraintsData;
        [HideInInspector] public ObiVolumeConstraintsData volumeConstraintsData;
         ...
         */
        public int particleCount => positions != null ? positions.Length : 0;
        ...
        public bool usesOrientedParticles =>
            invRotationalMasses != null && invRotationalMasses.Length > 0 &&
        ...
        {
            if (positions != null && index < positions.Length) return positions[index];
            return Vector3.zero;
        }

        public Quaternion GetParticleOrientation(int index)
        {
            if (orientations != null && index < orientations.Length) return orientations[index];
        ...
        {
            if (principalRadii != null && index < principalRadii.Length) return principalRadii[index][0];
            return 0;
        }
        ...
        public IEnumerator Generate()
        {
            ...
            foreach (var constraints in GetConstraints())
                for (var i = 0; i < constraints.GetBatchCount(); ++i)
                    constraints.GetBatch(i).initialActiveConstraintCount =
        ...
        public ObiParticleGroup InsertNewParticleGroup(string name, int index, bool saveImmediately = true)
        {
            ...
            {
                var group = CreateInstance<ObiParticleGroup>();
        ...
        private bool IsParticleSharedInConstraint(int index, List<int> particles, bool[] selected)
        {
            ...
            var containsUnselected = false;

            for (var k = 0; k < particles.Count; ++k)
            {
                containsCurrent |= particles[k] == index;
                containsUnselected |= !selected[particles[k]];

                if (containsCurrent && containsUnselected) return true;
            }

            return false;
        }

        private bool DoesParticleShareConstraints(IObiConstraints constraints, int index, List<int> particles,
            bool[] selected)
        {
            var shared = false;
            for (var i = 0; i < constraints.GetBatchCount(); ++i)
            {
                ...
                for (var j = 0; j < batch.activeConstraintCount; ++j)
                {
                    particles.Clear();
        ...
        {
            for (var j = 0; j < constraints.GetBatchCount(); ++j)
            {
                var batch = constraints.GetBatch(j);

                for (var i = batch.activeConstraintCount - 1; i >= 0; --i)
                {
                    particles.Clear();
        ...
        private void ParticlesSwappedInGroups(int index, int newIndex)
        {
            ...
            foreach (var group in groups)
                for (var i = 0; i < group.particleIndices.Count; ++i)
        ...
        public void RestoreRemovedParticles()
        {
            ...
            foreach (var constraints in GetConstraints())
                for (var j = 0; j < constraints.GetBatchCount(); ++j)
                    constraints.GetBatch(j).activeConstraintCount =
                        constraints.GetBatch(j).initialActiveConstraintCount;
        }
