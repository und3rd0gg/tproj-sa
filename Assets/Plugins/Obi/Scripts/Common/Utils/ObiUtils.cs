...
namespace Obi
{
    ...
    public static class ObiUtils
    {
        ...
        {
            var halfBodyLenght = bodyLenght * 0.5f;
            var halfBodyWidth = bodyWidth * 0.5f;

            // arrow body:
            Gizmos.DrawLine(new Vector3(halfBodyWidth, 0, -halfBodyLenght),
                new Vector3(halfBodyWidth, 0, halfBodyLenght));
            Gizmos.DrawLine(new Vector3(-halfBodyWidth, 0, -halfBodyLenght),
                new Vector3(-halfBodyWidth, 0, halfBodyLenght));
            Gizmos.DrawLine(new Vector3(-halfBodyWidth, 0, -halfBodyLenght),
                new Vector3(halfBodyWidth, 0, -halfBodyLenght));
        ...
        public static Vector3 ProjectPointLine(Vector3 point, Vector3 lineStart, Vector3 lineEnd, out float mu,
        ...
        public static bool LinePlaneIntersection(Vector3 planePoint, Vector3 planeNormal, Vector3 linePoint,
            Vector3 lineDirection, out Vector3 point)
        {
            point = linePoint;
            var lineNormal = lineDirection.normalized;
            var denom = Vector3.Dot(planeNormal, lineNormal);

            if (Mathf.Approximately(denom, 0))
                return false;

            var t = (Vector3.Dot(planeNormal, planePoint) - Vector3.Dot(planeNormal, linePoint)) / denom;
        ...
        public static float RaySphereIntersection(Vector3 rayOrigin, Vector3 rayDirection, Vector3 center, float radius)
        {
            ...
            var b = 2.0f * Vector3.Dot(oc, rayDirection);
            ...
            out Vector3 result)
        {
            ...
            var b0 = e0x * v0x + e0y * v0y + e0z * v0z;
            var b1 = e1x * v0x + e1y * v0y + e1z * v0z;
        ...
        {
            var darboux = Quaternion.Inverse(q1) * q2;
        ...
        {
            var center = (positionA + positionB + positionC) / 3;
            ...
            ref Vector3 bary)
        {
            ...
            var det = dot00 * dot11 - dot01 * dot01;
            if (Math.Abs(det) > 1E-38)
            {
                var u = (dot11 * dot02 - dot01 * dot12) / det;
                var v = (dot00 * dot12 - dot01 * dot02) / det;
                bary = new Vector3(1 - u - v, v, u);
            }
        }

        public static void BarycentricInterpolation(in Vector3 p1, in Vector3 p2, in Vector3 p3, in Vector3 coords,
            out Vector3 result)
        {
        ...
        public static Vector3[] CalculateAngleWeightedNormals(Vector3[] vertices, int[] triangles)
        {
            ...
            for (var i = 0; i < triangles.Length; i += 3)
            {
                ...
                e1 = v2 - v1;
                e2 = v3 - v1;
                normalBuffer[v1] += Vector3.Cross(e1, e2).normalized *
                                    Mathf.Acos(Vector3.Dot(e1.normalized, e2.normalized));

                e1 = v3 - v2;
                e2 = v1 - v2;
                normalBuffer[v2] += Vector3.Cross(e1, e2).normalized *
                                    Mathf.Acos(Vector3.Dot(e1.normalized, e2.normalized));

                e1 = v1 - v3;
                e2 = v2 - v3;
                normalBuffer[v3] += Vector3.Cross(e1, e2).normalized *
                                    Mathf.Acos(Vector3.Dot(e1.normalized, e2.normalized));
            }
        ...
        private static Vector3 unitOrthogonal(this Vector3 input)
        {
            ...
            {
                var invnm = 1 / Vector3.Magnitude(new Vector2(input.x, input.y));
                return new Vector3(-input.y * invnm, input.x * invnm, 0);
            }
            else
            {
                var invnm = 1 / Vector3.Magnitude(new Vector2(input.y, input.z));
        ...
        private static Vector3 EigenVector(Matrix4x4 D, float S)
        {
            ...
            var c0 = D.GetColumn(0);
            c0[0] -= S;
            var c1 = D.GetColumn(1);
            c1[1] -= S;
            var c2 = D.GetColumn(2);
            c2[2] -= S;

            // Use an upper triangle
            var c0p = new Vector3(c1[1] * c2[2] - c2[1] * c2[1], 0, 0);
            var c1p = new Vector3(c2[1] * c2[0] - c1[0] * c2[2], c0[0] * c2[2] - c2[0] * c2[0], 0);
            var c2p = new Vector3(c1[0] * c2[1] - c1[1] * c2[0], c1[0] * c2[0] - c0[0] * c2[1],
                c0[0] * c1[1] - c1[0] * c1[0]);

            // Get a column vector with a largest norm (non-zero).
            var C01s = c1p[0] * c1p[0];
            var C02s = c2p[0] * c2p[0];
            var C12s = c2p[1] * c2p[1];
            var norm = new Vector3(c0p[0] * c0p[0] + C01s + C02s,
        ...
        {
            var one_third = 1 / 3.0f;
            var one_sixth = 1 / 6.0f;
            var three_sqrt = Mathf.Sqrt(3.0f);

            Vector3 c0 = D.GetColumn(0);
            Vector3 c1 = D.GetColumn(1);
            Vector3 c2 = D.GetColumn(2);

            var m = one_third * (c0[0] + c1[1] + c2[2]);
            ...
            var q = 0.5f * (K00 * (K11 * K22 - K12s) - K22 * K01s - K11 * K02s) + c1[0] * c2[1] * c0[2];
            var p = one_sixth * (K00 * K00 + K11 * K11 + K22 * K22 + 2 * (K01s + K02s + K12s));

            var p_sqrt = Mathf.Sqrt(p);

            var tmp = p * p * p - q * q;
            var phi = one_third * Mathf.Atan2(Mathf.Sqrt(Mathf.Max(0, tmp)), q);
            var phi_c = Mathf.Cos(phi);
            var phi_s = Mathf.Sin(phi);
            var sqrt_p_c_phi = p_sqrt * phi_c;
            var sqrt_p_3_s_phi = p_sqrt * three_sqrt * phi_s;

            var e0 = m + 2 * sqrt_p_c_phi;
            var e1 = m - sqrt_p_c_phi - sqrt_p_3_s_phi;
            var e2 = m - sqrt_p_c_phi + sqrt_p_3_s_phi;
        ...
        public static void GetPointCloudAnisotropy(List<Vector3> points, float max_anisotropy, float radius,
            in Vector3 hint_normal, ref Vector3 centroid, ref Quaternion orientation, ref Vector3 principal_radii)
        {
