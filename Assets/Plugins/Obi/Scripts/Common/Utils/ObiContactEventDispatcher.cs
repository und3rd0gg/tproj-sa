...
namespace Obi
{
    ...
    public class ObiContactEventDispatcher : MonoBehaviour
    {
        ...
        private static int CompareByRef(ref Oni.Contact a, ref Oni.Contact b, ObiSolver solver)
        {
            if (a.bodyB == b.bodyB)
            {
                var hashA = solver.particleToActor[a.bodyA].actor.GetInstanceID();
                var hashB = solver.particleToActor[b.bodyA].actor.GetInstanceID();
        ...
        private void InvokeCallbacks(Oni.Contact[] data, int count)
        {
            ...
            while (a < lengthA && b < lengthB)
            {
                ...
                var compare = CompareByRef(ref data[a], ref prevData[b], solver);
                ...
                {
                    onContactStay.Invoke(solver, data[a++]);
                    b++;
                }
            }
        ...
        private void Solver_OnCollision(object sender, ObiSolver.ObiCollisionEventArgs args)
        {
            ...
            var filteredCount = FilterOutDistantContacts(args.contacts.Data, args.contacts.Count);
