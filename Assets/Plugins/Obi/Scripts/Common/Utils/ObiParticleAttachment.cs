...
namespace Obi
{
    ...
    public class ObiParticleAttachment : MonoBehaviour
    {
        ...
        [SerializeField] [HideInInspector] private bool m_ConstrainOrientation;
        [SerializeField] [HideInInspector] private float m_Compliance;
        ...
        [NonSerialized] private Quaternion[] m_OrientationOffsets;
        [NonSerialized] private Vector3[] m_PositionOffsets;
        ...
        /// </summary>
        public bool isBound => m_Target != null && m_SolverIndices != null && m_PositionOffsets != null;
            ...
            set
            {
                if (!Mathf.Approximately(value, m_Compliance))
                {
                    ...
                    if (m_AttachmentType == AttachmentType.Dynamic && pinBatch != null)
                        for (var i = 0; i < m_SolverIndices.Length; ++i)
        ...
        private void Bind()
        {
            ...
            {
                var bindMatrix = m_Target.worldToLocalMatrix * m_Actor.solver.transform.localToWorldMatrix;
                ...
                for (var i = 0; i < m_ParticleGroup.Count; ++i)
                {
                    var particleIndex = m_ParticleGroup.particleIndices[i];
                    if (particleIndex >= 0 && particleIndex < m_Actor.solverIndices.Length)
                    {
                        m_SolverIndices[i] = m_Actor.solverIndices[particleIndex];
                        m_PositionOffsets[i] =
                            bindMatrix.MultiplyPoint3x4(m_Actor.solver.positions[m_SolverIndices[i]]);
                    }
                    else
                    {
                        Debug.LogError("The particle group \'" + m_ParticleGroup.name +
                                       "\' references a particle that does not exist in the actor \'" + m_Actor.name +
                                       "\'.");
                ...
                {
                    var bindOrientation = bindMatrix.rotation;
            ...
            if (isBound)
                switch (type)
                {
                    case AttachmentType.Dynamic:
                        ...
                        {
                            var pins =
                                m_Actor.GetConstraintsByType(Oni.ConstraintType.Pin) as
                                    ObiConstraints<ObiPinConstraintsBatch>;
                            if (pins != null)
                            {
                    ...
                    case AttachmentType.Static:
                        ...
                        for (var i = 0; i < m_SolverIndices.Length; ++i)
                        {
                            var solverIndex = m_SolverIndices[i];
            ...
            if (enabled && m_Actor.isLoaded && isBound)
            {
                ...
                switch (m_AttachmentType)
                {
                    ...
                    case AttachmentType.Static:
                        ...
                        var blueprint = m_Actor.sourceBlueprint;
                        var targetActive = m_Target.gameObject.activeInHierarchy;

                        // Build the attachment matrix:
                        var attachmentMatrix = solver.transform.worldToLocalMatrix * m_Target.localToWorldMatrix;
                        ...
                        for (var i = 0; i < m_SolverIndices.Length; ++i)
                        {
                            ...
                            if (solverIndex >= 0 && solverIndex < solver.invMasses.count)
                            {
                                if (targetActive)
                                {
                                    ...
                                    solver.startPositions[solverIndex] = solver.positions[solverIndex] =
                                        attachmentMatrix.MultiplyPoint3x4(m_PositionOffsets[i]);
                                }
                                else
                                {
                                    solver.invMasses[solverIndex] = blueprint.invMasses[i];
                        ...
                        if (m_Actor.usesOrientedParticles && m_ConstrainOrientation)
                        {
                            var attachmentRotation = attachmentMatrix.rotation;
                            ...
                            for (var i = 0; i < m_SolverIndices.Length; ++i)
                            {
                                ...
                                if (solverIndex >= 0 && solverIndex < solver.invRotationalMasses.count)
                                {
                                    if (targetActive)
                                    {
                                        ...
                                        solver.startOrientations[solverIndex] = solver.orientations[solverIndex] =
                                            attachmentRotation * m_OrientationOffsets[i];
                                    }
                                    else
                                    {
                                        solver.invRotationalMasses[solverIndex] = blueprint.invRotationalMasses[i];
        ...
        private void BreakDynamicAttachment(float stepTime)
        {
            if (enabled && m_AttachmentType == AttachmentType.Dynamic && m_Actor.isLoaded && isBound)
            {
                ...
                var actorConstraints =
                    m_Actor.GetConstraintsByType(Oni.ConstraintType.Pin) as ObiConstraints<ObiPinConstraintsBatch>;
                var solverConstraints =
                    solver.GetConstraintsByType(Oni.ConstraintType.Pin) as ObiConstraints<ObiPinConstraintsBatch>;

                var dirty = false;
                if (actorConstraints != null && pinBatch != null)
                {
                    var pinBatchIndex = actorConstraints.batches.IndexOf(pinBatch);
                    if (pinBatchIndex >= 0 &&
                        pinBatchIndex < actor.solverBatchOffsets[(int) Oni.ConstraintType.Pin].Count)
                    {
                        var offset = actor.solverBatchOffsets[(int) Oni.ConstraintType.Pin][pinBatchIndex];
                        var solverBatch = solverConstraints.batches[pinBatchIndex];

                        var sqrTime = stepTime * stepTime;
                        for (var i = 0; i < pinBatch.activeConstraintCount; i++)
                        {
