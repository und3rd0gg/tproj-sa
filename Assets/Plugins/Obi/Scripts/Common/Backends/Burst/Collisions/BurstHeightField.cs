...
namespace Obi
{
    public struct BurstHeightField : BurstLocalOptimization.IDistanceFunction, IBurstCollider
    {
                              ...
                              float optimizationTolerance)
        {
            ...
            int2 min =
 new int2((int)math.floor(simplexBounds.min[0] / cellWidth), (int)math.floor(simplexBounds.min[2] / cellHeight));
            int2 max =
 new int2((int)math.floor(simplexBounds.max[0] / cellWidth), (int)math.floor(simplexBounds.max[2] / cellHeight));
                ...
                if (su >= 0 && su < resolutionU - 1)
                {
                    for (int sv = min[1]; sv <= max[1]; ++sv)
                    {
                        if (sv >= 0 && sv < resolutionV - 1)
                        {
                            ...
                            float h4 =
 heightFieldSamples[header.firstSample + csv1 * resolutionU + csu1] * shape.size.y;
                            ...
                            var colliderPoint =
 BurstLocalOptimization.Optimize<BurstHeightField>(ref this, positions, orientations, radii, simplices, simplexStart, simplexSize,
                            ...
                            if (rigidbodyIndex >= 0)
                                rbVelocity =
 BurstMath.GetRigidbodyVelocityAtPoint(rigidbodyIndex, colliderPoint.point, rigidbodies, solverToWorld);
