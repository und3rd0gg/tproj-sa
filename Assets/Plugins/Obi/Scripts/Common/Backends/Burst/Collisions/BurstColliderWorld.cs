    ...
    public class BurstColliderWorld : MonoBehaviour, IColliderWorldImpl
    {
        ...
        struct IdentifyMovingColliders : IJobParallelFor
        {
            ...
            public void Execute(int i)
            {
                ...
                BurstCellSpan newSpan =
 new BurstCellSpan(new int4(GridHash.Quantize(velocityBounds.min.xyz, cellSize), level),
                ...
                if (candidates.Length > 0)
                {
                    ...
                    for (int k = 0; k < uniqueCount; ++k)
                    {
                        ...
                        if (c < shapes.Length)
                        {
                            ...
                            {
                                var simplexCategory =
 filters[simplices[simplexStart + j]] & ObiUtils.FilterCategoryBitmask;
                                var simplexMask =
 (filters[simplices[simplexStart + j]] & ObiUtils.FilterMaskBitmask) >> 16;
                                shouldCollide |=
 (simplexCategory & colliderMask) != 0 && (simplexMask & colliderCategory) != 0;
                            }
                                          ...
                                          in BurstAabb simplexBoundsSS)
            {
                ...
                switch (shape.type)
                {
                    case ColliderShape.ShapeType.Sphere:
                        BurstSphere sphereShape = new BurstSphere() { colliderToSolver = colliderToSolver, shape =
 shape, dt = deltaTime };
                    ...
                    case ColliderShape.ShapeType.Box:
                        BurstBox boxShape = new BurstBox() { colliderToSolver = colliderToSolver, shape = shape, dt =
 deltaTime };
                    ...
                    case ColliderShape.ShapeType.Capsule:
                        BurstCapsule capsuleShape = new BurstCapsule(){colliderToSolver = colliderToSolver,shape =
 shape, dt = deltaTime };
                    ...
                    case ColliderShape.ShapeType.Heightmap:
                        ...
                        solverToCollider =
 math.inverse(float4x4.TRS(colliderToSolver.translation.xyz, colliderToSolver.rotation, colliderToSolver.scale.xyz));
