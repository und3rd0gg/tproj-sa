 ...
 math.mul(rigidbodies[rigidbodyIndex].inverseInertiaTensor, new float4(math.cross(r.xyz, impulseWS.xyz), 0));
        ...
        public static quaternion ExtractRotation(float4x4 matrix, quaternion rotation, int iterations)
        {
            ...
            for (int i = 0; i < iterations; ++i)
            {
                R = rotation.toMatrix();
                float3 omega =
 (math.cross(R.c0.xyz, matrix.c0.xyz) + math.cross(R.c1.xyz, matrix.c1.xyz) + math.cross(R.c2.xyz, matrix.c2.xyz)) /
        ...
        static float3 EigenVector(float3x3 D, float S)
        {
            ...
            float3 c2p =
 new float3(c1[0] * c2[1] - c1[1] * c2[0], c1[0] * c2[0] - c0[0] * c2[1], c0[0] * c1[1] - c1[0] * c1[0]);
