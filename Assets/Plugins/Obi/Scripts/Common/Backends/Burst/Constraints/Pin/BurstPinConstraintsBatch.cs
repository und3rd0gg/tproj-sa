...
namespace Obi
{
 ...
 math.lerp(prevPositions[particleIndex], positions[particleIndex], substeps);
                    ...
                    if (rigidbodyIndex >= 0)
                    {
                        ...
                        float4 velocityAtPoint =
 BurstMath.GetRigidbodyVelocityAtPoint(rigidbodyIndex, inertialFrame.frame.InverseTransformPoint(worldPinOffset), rigidbodies, rigidbodyLinearDeltas, rigidbodyAngularDeltas, inertialFrame.frame);
                        predictedPinOffset =
 BurstIntegration.IntegrateLinear(predictedPinOffset, inertialFrame.frame.TransformVector(velocityAtPoint), stepTime);

                        // predict rotation at the end of the step:
                        predictedRotation =
 BurstIntegration.IntegrateAngular(predictedRotation, rigidbody.angularVelocity + rigidbodyAngularDeltas[rigidbodyIndex], stepTime);
                    ...
                    float linearDLambda =
 (-constraint - compliances.x * lambda.w) / (invMasses[particleIndex] + rigidbodyLinearW + rigidbodyAngularW + compliances.x + BurstMath.epsilon);
                    ...
                    if (rigidbodyAngularW > 0 || invRotationalMasses[particleIndex] > 0)
                    {
                        ...
                        quaternion omega =
 math.mul(math.conjugate(orientations[particleIndex]), predictedRotation);   //darboux vector
                        ...
                        float3 dlambda =
 (omega.value.xyz - compliances.y * lambda.xyz) / new float3(compliances.y + invRotationalMasses[particleIndex] + rigidbodyAngularW + BurstMath.epsilon);
                        lambda.xyz += dlambda;
                        ...
                        orientDelta.value +=
 math.mul(predictedRotation, dlambdaQ).value * invRotationalMasses[particleIndex] / substeps;
