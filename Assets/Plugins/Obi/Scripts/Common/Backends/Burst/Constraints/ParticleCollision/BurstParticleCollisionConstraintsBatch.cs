...
namespace Obi
{
 ...
 (aMaterialIndex >= 0 ? collisionMaterials[aMaterialIndex].rollingContacts > 0 : false) |
 ...
 CombineCollisionMaterials(simplices[simplexStartA], simplices[simplexStartB]);
                    ...
                    for (int j = 0; j < simplexSizeA; ++j)
                    {
                        ...
                        simplexRadiusA +=
 BurstMath.EllipsoidRadius(contact.normal, orientations[particleIndex], radii[particleIndex].xyz) * contact.pointA[j];
                    }
                    for (int j = 0; j < simplexSizeB; ++j)
                    {
                        int particleIndex = simplices[simplexStartB + j];
 ...
 BurstMath.EllipsoidRadius(contact.normal, orientations[particleIndex], radii[particleIndex].xyz) * contact.pointA[j];
                    ...
                    // adhesion:
                    float lambda =
 contact.SolveAdhesion(posA, posB, material.stickDistance, material.stickiness, substepTime);
                    ...
                    {
                        float shock =
 solverParameters.shockPropagation * math.dot(contact.normal, math.normalizesafe(gravity));
 ...
 delta * invMasses[particleIndex] * contact.pointA[j] * baryScale * (1 - shock);
                        ...
                        for (int j = 0; j < simplexSizeB; ++j)
                        {
                            int particleIndex = simplices[simplexStartB + j];
                            deltas[particleIndex] -=
 delta * invMasses[particleIndex] * contact.pointB[j] * baryScale * (1 + shock);
                            counts[particleIndex]++;
                        }
                    }
