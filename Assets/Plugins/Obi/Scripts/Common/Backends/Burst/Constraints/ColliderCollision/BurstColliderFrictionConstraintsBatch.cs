...
namespace Obi
{
 ...
 ObiColliderWorld.GetInstance().collisionMaterials.AsNativeArray<BurstCollisionMaterial>(),
                ...
                rigidbodyAngularDeltas =
 solverImplementation.abstraction.rigidbodyAngularDeltas.AsNativeArray<float4>(),
 ...
 BurstMath.EllipsoidRadius(contact.normal, prevOrientations[particleIndex], radii[particleIndex].xyz) * contact.pointA[j];
                    ...
                    if (rigidbodyIndex >= 0)
                    {
                        ...
                        relativeVelocity -=
 BurstMath.GetRigidbodyVelocityAtPoint(rigidbodyIndex, contact.pointB, rigidbodies, rigidbodyLinearDeltas, rigidbodyAngularDeltas, inertialFrame.frame);
                    }

                    // Determine impulse magnitude:
                    float2 impulses =
 contact.SolveFriction(relativeVelocity, material.staticFriction, material.dynamicFriction, stepTime);
 ...
 math.length(math.mul(solverInertiaA, math.normalizesafe(angularVelocityA)));
                            ...
                            {
                                angVelDeltaB =
 math.mul(-rigidbodies[rigidbodyIndex].inverseInertiaTensor, new float4(math.cross(rB.xyz, totalImpulse.xyz), 0));
                                angularVelocityB = rigidbodies[rigidbodyIndex].angularVelocity + angVelDeltaB;
                                invMassB =
 math.length(math.mul(rigidbodies[rigidbodyIndex].inverseInertiaTensor, math.normalizesafe(angularVelocityB)));
                            }
