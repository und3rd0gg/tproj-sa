    ...
    public class BurstDensityConstraints : BurstConstraintsImpl<BurstDensityConstraintsBatch>
    {
        ...
        private JobHandle UpdateInteractions(JobHandle inputDeps)
        {
            ...
            var updateInteractions = new UpdateInteractionsJob()
            {
                ...
                densityKernel =
 new Poly6Kernel(((BurstSolverImpl)solver).abstraction.parameters.mode == Oni.SolverParameters.Mode.Mode2D),
                gradientKernel =
 new SpikyKernel(((BurstSolverImpl)solver).abstraction.parameters.mode == Oni.SolverParameters.Mode.Mode2D),
            };
        ...
        private JobHandle CalculateLambdas(JobHandle inputDeps, float deltaTime)
        {
            ...
            var calculateLambdas = new CalculateLambdasJob()
            {
                ...
                densityKernel =
 new Poly6Kernel(m_Solver.abstraction.parameters.mode == Oni.SolverParameters.Mode.Mode2D),
                gradientKernel =
 new SpikyKernel(m_Solver.abstraction.parameters.mode == Oni.SolverParameters.Mode.Mode2D),
        ...
        public struct ApplyVorticityConfinementAndAtmosphere : IJobParallelFor
        {
            ...
            public void Execute(int p)
            {
                ...
                velocities[i] -=
 atmosphericDrag[i] * velocityDiff * math.max(0, 1 - fluidData[i][0] / restDensities[i]) * dt;
                ...
                velocities[i] +=
 new float4(math.cross(math.normalizesafe(eta[i]).xyz,vorticities[i].xyz), 0) * vorticityConfinement[i] * dt;
            }
        }
