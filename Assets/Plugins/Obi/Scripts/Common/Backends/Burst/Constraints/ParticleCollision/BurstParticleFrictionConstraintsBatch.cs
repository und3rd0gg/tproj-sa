        ...
        public struct ParticleFrictionConstraintsBatchJob : IJobParallelFor
        {
            ...
            public void Execute(int workItemIndex)
            {
                ...
                for (int i = start; i < end; ++i)
                {
                    ...
                    BurstCollisionMaterial material =
 CombineCollisionMaterials(simplices[simplexStartA], simplices[simplexStartB]);
                    ...
                    for (int j = 0; j < simplexSizeA; ++j)
                    {
                        ...
                        linearVelocityA +=
 BurstIntegration.DifferentiateLinear(positions[particleIndex], prevPositions[particleIndex], substepTime) * contact.pointA[j];
                        angularVelocityA +=
 BurstIntegration.DifferentiateAngular(orientations[particleIndex], prevOrientations[particleIndex], substepTime) * contact.pointA[j];
                        ...
                        simplexRadiusA +=
 BurstMath.EllipsoidRadius(contact.normal, prevOrientations[particleIndex], radii[particleIndex].xyz) * contact.pointA[j];
                    }
                    for (int j = 0; j < simplexSizeB; ++j)
                    {
                        ...
                        linearVelocityB +=
 BurstIntegration.DifferentiateLinear(positions[particleIndex], prevPositions[particleIndex], substepTime) * contact.pointB[j];
                        angularVelocityB +=
 BurstIntegration.DifferentiateAngular(orientations[particleIndex], prevOrientations[particleIndex], substepTime) * contact.pointB[j];
                        ...
                        simplexRadiusB +=
 BurstMath.EllipsoidRadius(contact.normal, prevOrientations[particleIndex], radii[particleIndex].xyz) * contact.pointB[j];
                    }
                    ...
                    float2 impulses =
 contact.SolveFriction(relativeVelocity, material.staticFriction, material.dynamicFriction, substepTime);
                    ...
                    if (math.abs(impulses.x) > BurstMath.epsilon || math.abs(impulses.y) > BurstMath.epsilon)
                    {
                        ...
                        {
                            int particleIndex = simplices[simplexStartA + j];
                            deltas[particleIndex] +=
 (tangentImpulse * contact.tangentInvMassA + bitangentImpulse * contact.bitangentInvMassA) * substepTime * contact.pointA[j] * baryScale;
                        ...
                        for (int j = 0; j < simplexSizeB; ++j)
                        {
                            int particleIndex = simplices[simplexStartB + j];
                            deltas[particleIndex] -=
 (tangentImpulse * contact.tangentInvMassB + bitangentImpulse * contact.bitangentInvMassB) * substepTime * contact.pointB[j] * baryScale;
                            counts[particleIndex]++;
                        }
                        ...
                        if (material.rollingContacts > 0)
                        {
                            ...
                            float4 angVelDeltaA =
 math.mul(solverInertiaA, new float4(math.cross(rA.xyz, totalImpulse.xyz), 0));
                            float4 angVelDeltaB =
 -math.mul(solverInertiaB, new float4(math.cross(rB.xyz, totalImpulse.xyz), 0));
                            ...
                            float invMassA =
 math.length(math.mul(solverInertiaA, math.normalizesafe(angularVelocityA)));
                            float invMassB =
 math.length(math.mul(solverInertiaB, math.normalizesafe(angularVelocityB)));

                            // Calculate rolling axis and angular velocity deltas:
                            float4 rollAxis = float4.zero;
                            float rollingImpulse =
 contact.SolveRollingFriction(angularVelocityA, angularVelocityB, material.rollingFriction, invMassA, invMassB, ref rollAxis);
                            ...
                            quaternion orientationDeltaA =
 BurstIntegration.AngularVelocityToSpinQuaternion(orientationA, angVelDeltaA, substepTime);
                            quaternion orientationDeltaB =
 BurstIntegration.AngularVelocityToSpinQuaternion(orientationB, angVelDeltaB, substepTime);
