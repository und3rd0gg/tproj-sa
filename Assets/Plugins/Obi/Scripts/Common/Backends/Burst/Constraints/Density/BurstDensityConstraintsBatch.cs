...
namespace Obi
{
 ...
 (diffusion[pair.particleA] + diffusion[pair.particleB]) * pair.avgKernel * dt;
        ...
        public struct ApplyDensityConstraintsJob : IJobParallelFor
        {
            ...
            public void Execute(int workItemIndex)
            {
                ...
                for (int i = start; i < end; ++i)
                {
                    ...
                    float wAvg =
 pair.avgKernel / ((densityKernel.W(0, radii[pair.particleA]) + densityKernel.W(0, radii[pair.particleB])) * 0.5f);
                    float scorrA =
 -(0.001f + 0.2f * surfaceTension[pair.particleA]) * wAvg / (invMasses[pair.particleA] * fluidData[pair.particleA][3]);
                    float scorrB =
 -(0.001f + 0.2f * surfaceTension[pair.particleB]) * wAvg / (invMasses[pair.particleB] * fluidData[pair.particleB][3]);

                    // calculate position delta:
                    float4 delta =
 pair.gradient * pair.avgGradient * ((fluidData[pair.particleA][1] + scorrA) * restVolumeB + (fluidData[pair.particleB][1] + scorrB) * restVolumeA) * sorFactor;
        ...
        public struct NormalsViscosityAndVorticityJob : IJobParallelFor
        {
            ...
            public void Execute(int workItemIndex)
            {
                ...
                for (int i = start; i < end; ++i)
                {
                    ...
                    normals[pair.particleA] +=
 vgrad * radius / invMasses[pair.particleB] / fluidData[pair.particleB][0];
                    normals[pair.particleB] -=
 vgrad * radius / invMasses[pair.particleA] / fluidData[pair.particleA][0];
                }
            }
        }
        ...
        public struct CalculateVorticityEta : IJobParallelFor
        {
            ...
            public void Execute(int workItemIndex)
            {
                ...
                for (int i = start; i < end; ++i)
                {
                    ...
                    float4 vgrad = pair.gradient * pair.avgGradient;
                    eta[pair.particleA] +=
 math.length(vorticities[pair.particleA]) * vgrad / invMasses[pair.particleB] / restDensities[pair.particleB];
                    eta[pair.particleB] -=
 math.length(vorticities[pair.particleB]) * vgrad / invMasses[pair.particleA] / restDensities[pair.particleA];
                }
            }
        }
        ...
        public struct AccumulateSmoothPositionsJob : IJobParallelFor
        {
            ...
            public void Execute(int workItemIndex)
            {
                ...
                for (int i = start; i < end; ++i)
                {
                    ...
                    smoothPositions[pair.particleA] +=
 new float4(renderablePositions[pair.particleB].xyz,1) * pair.avgKernel;
                    smoothPositions[pair.particleB] +=
 new float4(renderablePositions[pair.particleA].xyz,1) * pair.avgKernel;
