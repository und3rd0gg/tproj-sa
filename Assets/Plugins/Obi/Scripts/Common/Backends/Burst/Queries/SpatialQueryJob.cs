...
namespace Obi
{
    ...
    unsafe struct SpatialQueryJob : IJobParallelFor
    {
                           ...
                           int simplexSize)
        {
            ...
            {
                case QueryShape.QueryType.Sphere:
                    BurstSphereQuery sphereShape = new BurstSphereQuery() { colliderToSolver = shapeToSolver, shape =
 shape};
    ...
    public struct CalculateQueryDistances : IJobParallelFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for (int j = 0; j < simplexSize; ++j)
            {
                ...
                simplexRadius +=
 BurstMath.EllipsoidRadius(result.normal, prevOrientations[particleIndex], radii[particleIndex].xyz) * result.simplexBary[j];
            }
