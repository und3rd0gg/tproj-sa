    ...
    public class ParticleGrid : IDisposable
    {
        ...
        public struct GenerateParticleParticleContactsJob : IJobParallelFor
        {
            ...
            private void InteractionTest(int A, int B, ref BurstSimplex simplexShape)
            {
                ...
                int groupA =
 GetSimplexGroup(simplexStartA, simplexSizeA, out ObiUtils.ParticleFlags flagsA, out int categoryA, out int maskA, ref restPositionsEnabled);
                int groupB =
 GetSimplexGroup(simplexStartB, simplexSizeB, out ObiUtils.ParticleFlags flagsB, out int categoryB, out int maskB, ref restPositionsEnabled);
                ...
                if ((flagsA & ObiUtils.ParticleFlags.Fluid) != 0 && (flagsB & ObiUtils.ParticleFlags.Fluid) != 0)
                {
                    ...
                    {
                        fluidInteractionsQueue.Enqueue(new FluidInteraction { particleA = particleA, particleB =
 particleB });
                    }
                }
                else // at least one solid particle is present:
                {
                    ...
                    {
                        var restPoint =
 BurstLocalOptimization.Optimize<BurstSimplex>(ref simplexShape, restPositions, restOrientations, radii,
                    ...
                    var surfacePoint =
 BurstLocalOptimization.Optimize<BurstSimplex>(ref simplexShape, positions, orientations, radii,
