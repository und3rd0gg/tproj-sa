...
namespace Obi
{
    ...
    {
        public ObiHeightFieldHandle(TerrainData data, int index = -1) : base(index)
        {
            owner = data;
        }
    }
    ...
    public class ObiHeightFieldContainer
    {
        ...
        public ObiHeightFieldHandle GetOrCreateHeightField(TerrainData source)
        {
            ...
            if (!handles.TryGetValue(source, out handle))
            {
                ...
                var width = source.heightmapResolution;
                var height = source.heightmapResolution;

                var heights = source.GetHeights(0, 0, width, height);
