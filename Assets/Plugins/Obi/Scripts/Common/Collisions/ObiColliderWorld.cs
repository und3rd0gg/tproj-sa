...
namespace Obi
{
    ...
    public class ObiColliderWorld
    {
        ...
        [NonSerialized]
        public List<ObiColliderHandle>
            colliderHandles; // list of collider handles, used by ObiCollider components to retrieve them.
        ...
        [NonSerialized]
        public List<ObiRigidbodyHandle>
            rigidbodyHandles; // list of rigidbody handles, used by ObiRigidbody components to retrieve them.
        ...
        private void DestroyIfUnused()
        {
            ...
            if (colliderHandles.Count == 0 && rigidbodyHandles.Count == 0 && materialHandles.Count == 0 &&
        ...
        public void DestroyCollider(ObiColliderHandle handle)
        {
            if (colliderShapes != null && handle != null && handle.isValid && handle.index < colliderHandles.Count)
            {
                ...
                var lastIndex = colliderHandles.Count - 1;
        ...
        public void DestroyRigidbody(ObiRigidbodyHandle handle)
        {
            ...
            {
                var index = handle.index;
                var lastIndex = rigidbodyHandles.Count - 1;
        ...
        public void DestroyCollisionMaterial(ObiCollisionMaterialHandle handle)
        {
            ...
            {
                var index = handle.index;
                var lastIndex = materialHandles.Count - 1;
        ...
        public void UpdateColliders()
        {
            ...
            for (var i = 0; i < colliderHandles.Count; ++i)
        ...
        public void UpdateRigidbodies(List<ObiSolver> solvers, float stepTime)
        {
            ...
            for (var i = 0; i < rigidbodyHandles.Count; ++i)
        ...
        public void UpdateWorld(float deltaTime)
        {
            for (var i = 0; i < implementations.Count; ++i)
                if (implementations[i].referenceCount > 0)
                {
                    ...
                    implementations[i].SetTriangleMeshData(triangleMeshContainer.headers,
                        triangleMeshContainer.bihNodes, triangleMeshContainer.triangles,
                        triangleMeshContainer.vertices);
                    implementations[i].SetEdgeMeshData(edgeMeshContainer.headers, edgeMeshContainer.bihNodes,
                        edgeMeshContainer.edges, edgeMeshContainer.vertices);
                    implementations[i]
                        .SetDistanceFieldData(distanceFieldContainer.headers, distanceFieldContainer.dfNodes);
        ...
        public void UpdateRigidbodyVelocities(List<ObiSolver> solvers)
        {
            ...
            if (count > 0)
            {
                ...
                var rcpCount = 1.0f / count;

                for (var i = 0; i < rigidbodyHandles.Count; ++i)
                {
                    var linearDelta = Vector4.zero;
                    var angularDelta = Vector4.zero;
