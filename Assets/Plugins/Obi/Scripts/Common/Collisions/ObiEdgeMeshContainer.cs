...
namespace Obi
{
    ...
    {
        public ObiEdgeMeshHandle(EdgeCollider2D collider, int index = -1) : base(index)
        {
            owner = collider;
        }
    }
    ...
    public struct EdgeMeshHeader
    {
        ...
        public EdgeMeshHeader(int firstNode, int nodeCount, int firstTriangle, int triangleCount, int firstVertex,
            int vertexCount)
        {
    ...
    public class ObiEdgeMeshContainer
    {
        ...
        public ObiEdgeMeshHandle GetOrCreateEdgeMesh(EdgeCollider2D source)
        {
            ...
            {
                var sourceVertices = source.points;
                var sourceEdges = new int[source.edgeCount * 2];

                for (var i = 0; i < source.edgeCount; ++i)
                {
                    sourceEdges[i * 2] = i;
                ...
                var t = new IBounded[source.edgeCount];
                for (var i = 0; i < source.edgeCount; ++i)
                    t[i] = new Edge(i, i + 1, sourceVertices[i], sourceVertices[i + 1]);
                var sourceBih = BIH.Build(ref t);

                var edgs = Array.ConvertAll(t, x => (Edge) x);

                handle = new ObiEdgeMeshHandle(source, headers.count);
                handles.Add(source, handle);
                headers.Add(new EdgeMeshHeader(bihNodes.count, sourceBih.Length, edges.count, edgs.Length,
