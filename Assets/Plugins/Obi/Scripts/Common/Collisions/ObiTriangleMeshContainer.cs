...
namespace Obi
{
    ...
    {
        public ObiTriangleMeshHandle(Mesh mesh, int index = -1) : base(index)
        {
            owner = mesh;
        }
    }
    ...
    public struct TriangleMeshHeader //we need to use the header in the backend, so it must be a struct.
    {
        ...
        public TriangleMeshHeader(int firstNode, int nodeCount, int firstTriangle, int triangleCount, int firstVertex,
            int vertexCount)
        {
    ...
    public class ObiTriangleMeshContainer
    {
        ...
        {
            var handle = new ObiTriangleMeshHandle(null);
            ...
            if (source != null && !handles.TryGetValue(source, out handle))
            {
                ...
                var t = new IBounded[sourceTris.Length / 3];
                for (var i = 0; i < t.Length; ++i)
                {
                    var t1 = sourceTris[i * 3];
                    var t2 = sourceTris[i * 3 + 1];
                    var t3 = sourceTris[i * 3 + 2];
                ...
                headers.Add(new TriangleMeshHeader(bihNodes.count, sourceBih.Length, triangles.count, tris.Length,
