...
namespace Obi
{
    public class BIH
    {
        ...
        {
            var nodes = new List<BIHNode> {new BIHNode(0, elements.Length)};
            ...
            while (queue.Count > 0)
            {
                ...
                if (node.count > 2)
                {
                    ...
                    var end = start + (node.count - 1);

                    // calculate bounding box of all elements:
                    var b = elements[start].GetBounds();
                    for (var k = start + 1; k <= end; ++k)
                        b.Encapsulate(elements[k].GetBounds());

                    // determine split axis (longest one):
                    Vector3 size = b.size;
                    var axis = node.axis = size.x > size.y ? size.x > size.z ? 0 : 2 :
                        size.y > size.z ? 1 : 2;

                    // place split plane at half the longest axis:
                    var pivot = b.min[axis] + size[axis] * 0.5f;

                    // partition elements according to which side of the split plane they're at:
                    var j = HoarePartition(elements, start, end, pivot, ref node, axis);
                    ...
                    var overlap = size[axis] > 0 ? Mathf.Max(node.min - node.max, 0) / size[axis] : 1;
        ...
        public static int HoarePartition(IBounded[] elements, int start, int end, float pivot, ref BIHNode node,
            int axis)
        {
            var i = start;
            var j = end;
            ...
            in Vector3 point)
        {
            ...
            for (var i = node.start; i < node.start + node.count; ++i)
            {
                ...
                var pointToTri = point - pointOnTri;
                var sqrDistance = pointToTri.sqrMagnitude;

                if (sqrDistance < minDistance)
                {
                    var bary = Vector3.zero;
                    ObiUtils.BarycentricCoordinates(in vertices[t.i1], in vertices[t.i2], in vertices[t.i3],
                        in pointOnTri, ref bary);
                    ...
                    sign = (pointToTri.x * interpolatedNormal.x +
                            pointToTri.y * interpolatedNormal.y +
                            pointToTri.z * interpolatedNormal.z).PureSign();
            ...
            in Vector3 point)
        {
            ...
            if (node.firstChild >= 0)
            {
                ...
                if (node.min > node.max)
                {
                    ...
                    {
                        si = MinSignedDistance(
                            DistanceToSurface(nodes, triangles, vertices, normals, in nodes[node.firstChild], in point),
                            DistanceToSurface(nodes, triangles, vertices, normals, in nodes[node.firstChild + 1],
                                in point));
                    }
                    // CASE 2: to the right of left pivot, that is: in the right child only.
                    else if (p > node.min)
                    {
                        si = DistanceToSurface(nodes, triangles, vertices, normals, in nodes[node.firstChild + 1],
                            in point);
                        ...
                        if (Mathf.Abs(si) > Mathf.Abs(p - node.min))
                            si = MinSignedDistance(si,
                                DistanceToSurface(nodes, triangles, vertices, normals, in nodes[node.firstChild],
                                    in point));
                    }
                ...
                else
                {
                    ...
                    {
                        si = DistanceToSurface(nodes, triangles, vertices, normals, in nodes[node.firstChild],
                            in point);
