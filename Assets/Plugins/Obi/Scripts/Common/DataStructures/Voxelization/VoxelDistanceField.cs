...
namespace Obi
{
    ...
    public class VoxelDistanceField
    {
        ...
        public float SampleUnfiltered(int x, int y, int z)
        {
            ...
            var dist = Vector3.Distance(voxelizer.GetVoxelCenter(distanceField[x, y, z]),
        ...
        public Vector4 SampleFiltered(float x, float y, float z)
        {
            ...
            var max = voxelizer.GetVoxelCenter(new Vector3Int(voxelizer.resolution.x - 1, voxelizer.resolution.y - 1,
                voxelizer.resolution.z - 1));
            ...
            var X1 = Mathf.Lerp(xz00, xz10, norm.x);
            var X2 = Mathf.Lerp(xz01, xz11, norm.x);
            var X3 = Mathf.Lerp(yz00, yz10, norm.x);
            var X4 = Mathf.Lerp(yz01, yz11, norm.x);

            var Y1 = Mathf.Lerp(X1, X2, norm.z);
            var Y2 = Mathf.Lerp(X3, X4, norm.z);

            var R = Mathf.Lerp(Mathf.Lerp(xz10, xz11, norm.z), Mathf.Lerp(yz10, yz11, norm.z), norm.y);
            var L = Mathf.Lerp(Mathf.Lerp(xz00, xz01, norm.z), Mathf.Lerp(yz00, yz01, norm.z), norm.y);

            var F = Mathf.Lerp(X2, X4, norm.y);
            var B = Mathf.Lerp(X1, X3, norm.y);
        ...
        public IEnumerator JumpFlood()
        {
            ...
            var auxBuffer = new Vector3Int[voxelizer.resolution.x,
                voxelizer.resolution.y,
                voxelizer.resolution.z];

            // initialize distance field:
            for (var x = 0; x < distanceField.GetLength(0); ++x)
            for (var y = 0; y < distanceField.GetLength(1); ++y)
            for (var z = 0; z < distanceField.GetLength(2); ++z)
            ...
            var size = Mathf.Max(distanceField.GetLength(0),
                distanceField.GetLength(1),
                distanceField.GetLength(2));
            var step = (int) (size / 2.0f);
        ...
        private void JumpFloodPass(int stride, Vector3Int[,,] input, Vector3Int[,,] output)
        {
            ...
            for (var z = 0; z < input.GetLength(2); ++z)
            {
                // our position:
