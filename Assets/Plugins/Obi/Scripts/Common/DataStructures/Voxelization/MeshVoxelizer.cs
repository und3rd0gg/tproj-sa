...
namespace Obi
{
    ...
    public class MeshVoxelizer
    {
        ...
        public static readonly Vector3Int[] fullNeighborhood =
        {
            // face neighbors: 
        ...
        public static readonly Vector3Int[] edgefaceNeighborhood =
        {
            new Vector3Int(-1, -1, 0),
            new Vector3Int(-1, 0, -1),
            new Vector3Int(-1, 0, 0),
        ...
        public static readonly Vector3Int[] faceNeighborhood =
        {
            new Vector3Int(-1, 0, 0),
            new Vector3Int(1, 0, 0),
            new Vector3Int(0, -1, 0),
        ...
        public static readonly Vector3Int[] edgeNeighborhood =
        {
            new Vector3Int(-1, -1, 0),
            new Vector3Int(-1, 0, -1),
            new Vector3Int(-1, 0, 1),
        ...
        public static readonly Vector3Int[] vertexNeighborhood =
        {
            new Vector3Int(-1, -1, -1),
            new Vector3Int(-1, -1, 1),
            new Vector3Int(-1, 1, -1),
        ...
        {
            get => voxels[GetVoxelIndex(x, y, z)];
        ...
        {
            var b = new Bounds(v1, Vector3.zero);
            b.Encapsulate(v2);
            b.Encapsulate(v3);
            return b;
        }
        ...
        private void AppendOverlappingVoxels(in Bounds bounds, in Vector3 v1, in Vector3 v2, in Vector3 v3,
            int triangleIndex)
        {
            var min = GetPointVoxel(bounds.min);
            var max = GetPointVoxel(bounds.max);

            for (var x = min.x; x <= max.x; ++x)
            for (var y = min.y; y <= max.y; ++y)
            for (var z = min.z; z <= max.z; ++z)
            {
                var voxel = new Bounds(new Vector3(x + 0.5f, y + 0.5f, z + 0.5f) * voxelSize, Vector3.one * voxelSize);

                if (IsIntersecting(voxel, v1, v2, v3))
                {
                    var index = GetVoxelIndex(x - origin.x, y - origin.y, z - origin.z);
        ...
        public IEnumerator Voxelize(Matrix4x4 transform, bool generateTriangleIndices = false)
        {
            ...
            var max = GetPointVoxel(xfBounds.max) + new Vector3Int(1, 1, 1);
            ...
            var triIndices = input.triangles;
            var vertices = input.vertices;

            // Generate surface voxels:
            for (var i = 0; i < triIndices.Length; i += 3)
            {
                var v1 = transform.MultiplyPoint3x4(vertices[triIndices[i]]);
                var v2 = transform.MultiplyPoint3x4(vertices[triIndices[i + 1]]);
                var v3 = transform.MultiplyPoint3x4(vertices[triIndices[i + 2]]);

                var triBounds = GetTriangleBounds(v1, v2, v3);
        ...
        public void BoundaryThinning()
        {
            ...
            {
                var sum = 0;
                for (var j = 0; j < faceNeighborhood.Length; ++j)
                {
                    var index = faceNeighborhood[j];
                    if (VoxelExists(index.x + x, index.y + y, index.z + z) &&
                        this[index.x + x, index.y + y, index.z + z] != Voxel.Outside) sum++;
                }
        ...
        private static bool TriangleAabbSATTest(in Vector3 v0, in Vector3 v1, in Vector3 v2, in Vector3 aabbExtents,
            in Vector3 axis)
        {
            ...
            var r = aabbExtents.x * Mathf.Abs(axis.x) +
                    aabbExtents.y * Mathf.Abs(axis.y) +
                    aabbExtents.z * Mathf.Abs(axis.z);

            var maxP = Mathf.Max(p0, Mathf.Max(p1, p2));
            var minP = Mathf.Min(p0, Mathf.Min(p1, p2));
