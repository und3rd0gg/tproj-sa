...
namespace Obi
{
    public struct ColliderRigidbody
    {
        ...
        {
            var kinematic = !Application.isPlaying || rb.isKinematic || kinematicForParticles;
            ...
            var inertiaTensor = kinematic ? Vector3.zero : invTensor;

            // calculate full world space inertia matrix:
            var rotation = Matrix4x4.Rotate(rb.rotation * rb.inertiaTensorRotation);
        ...
        public void FromRigidbody(Rigidbody2D rb, bool kinematicForParticles)
        {
            ...
            var inertiaTensor = kinematic
                ? Vector3.zero
                : new Vector3(0, 0, (rb.constraints & RigidbodyConstraints2D.FreezeRotation) != 0 ? 0 : 1 / rb.inertia);

            var rotation = Matrix4x4.Rotate(Quaternion.AngleAxis(rb.rotation, Vector3.forward));
            inverseInertiaTensor = rotation * Matrix4x4.Scale(inertiaTensor) * rotation.transpose;
