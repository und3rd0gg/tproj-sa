...
namespace Obi
{
    public struct ObiPathFrame
    {
        ...
        public ObiPathFrame(Vector3 position, Vector3 tangent, Vector3 normal, Vector3 binormal, Vector4 color,
            float thickness)
        {
        ...
        {
            return new ObiPathFrame(c1.position + c2.position, c1.tangent + c2.tangent, c1.normal + c2.normal,
                c1.binormal + c2.binormal, c1.color + c2.color, c1.thickness + c2.thickness);
        }

        public static ObiPathFrame operator *(float f, ObiPathFrame c)
        {
            return new ObiPathFrame(c.position * f, c.tangent * f, c.normal * f, c.binormal * f, c.color * f,
                c.thickness * f);
        }

        public static void WeightedSum(float w1, float w2, float w3, ref ObiPathFrame c1, ref ObiPathFrame c2,
            ref ObiPathFrame c3, ref ObiPathFrame sum)
        {
        ...
        {
            var twistQ = Quaternion.AngleAxis(twist, tangent);
        ...
        public void Transport(ObiPathFrame frame, float twist)
        {
            ...
            var rotQ = Quaternion.FromToRotation(tangent, frame.tangent);
            var twistQ = Quaternion.AngleAxis(twist, frame.tangent);
            var finalQ = twistQ * rotQ;
        ...
        {
            // Calculate delta rotation:
            var rotQ = Quaternion.FromToRotation(tangent, newTangent);
            var twistQ = Quaternion.AngleAxis(twist, newTangent);
            var finalQ = twistQ * rotQ;
        ...
        public Matrix4x4 ToMatrix(Axis mainAxis)
        {
            ...
            var xo = (int) mainAxis % 3 * 4;
            var yo = ((int) mainAxis + 1) % 3 * 4;
            var zo = ((int) mainAxis + 2) % 3 * 4;
