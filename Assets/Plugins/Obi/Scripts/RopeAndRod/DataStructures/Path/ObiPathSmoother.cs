...
namespace Obi
{
    ...
    {
        private static ProfilerMarker m_AllocateRawChunksPerfMarker = new ProfilerMarker("AllocateRawChunks");
        private static ProfilerMarker m_GenerateSmoothChunksPerfMarker = new ProfilerMarker("GenerateSmoothChunks");
        ...
        private readonly BitArray decimateBitArray = new BitArray(0);
        ...
        private readonly Stack<Vector2Int> stack = new Stack<Vector2Int>();

        private Matrix4x4 w2l;
        private Quaternion w2lRotation;

        public float SmoothLength => smoothLength;

        public float SmoothSections => smoothSections;
        ...
        private void AllocateRawChunks(ObiRopeBase actor)
        {
            using (m_AllocateRawChunksPerfMarker.Auto())
            {
                ...
                var particles = 0;
                for (var i = 0; i < actor.elements.Count; ++i)
                {
                    particles++;
            ...
            bool interpolateOrientation = true)
        {
            ...
            {
                var current = actor.GetParticleOrientation(particleIndex);
                var previous = actor.GetParticleOrientation(Mathf.Max(0, particleIndex - 1));
                var average = w2lRotation *
                              (interpolateOrientation ? Quaternion.SlerpUnclamped(current, previous, 0.5f) : current);
        ...
        public void GenerateSmoothChunks(ObiRopeBase actor, uint smoothingLevels)
        {
            using (m_GenerateSmoothChunksPerfMarker.Auto())
            {
                ...
                var frame_0 = new ObiPathFrame(); // "next" frame
                var frame_1 = new ObiPathFrame(); // current frame
                var frame_2 = new ObiPathFrame(); // previous frame

                // generate curve for each rope chunk:
                for (var i = 0; i < rawChunks.Count; ++i)
                {
                    var elementCount = rawChunks[i].Count - 1;
                    ...
                    for (var m = 1; m <= rawChunks[i].Count; ++m)
                    {
                        int index;
                        if (m >= elementCount)
        ...
        {
            var edgeMu = smoothSections * Mathf.Clamp(mu, 0, 0.9999f);
            var index = (int) edgeMu;
            var sectionMu = edgeMu - index;

            var counter = 0;
            var chunkIndex = -1;
            var indexInChunk = -1;
            for (var i = 0; i < smoothChunks.Count; ++i)
            {
            ...
            var chunk = smoothChunks[chunkIndex];
            var s1 = chunk[indexInChunk];
            var s2 = chunk[Mathf.Min(indexInChunk + 1, chunk.Count - 1)];
        ...
        private bool Decimate(ObiList<ObiPathFrame> input, ObiList<ObiPathFrame> output, float threshold)
        {
            ...
            var scaledThreshold = threshold * threshold * 0.01f;
            ...
            while (stack.Count > 0)
            {
                ...
                for (var i = index + 1; i < range.y; ++i)
                    if (decimateBitArray[i])
                    {
                        var d = Vector3.SqrMagnitude(ObiUtils.ProjectPointLine(input[i].position,
        ...
        private void Chaikin(ObiList<ObiPathFrame> input, ObiList<ObiPathFrame> output, uint k)
        {
            ...
            var pCount = (int) Mathf.Pow(2, k);

            // precalculate some quantities:
            var n0 = input.Count - 1;
            var twoRaisedToMinusKPlus1 = Mathf.Pow(2, -(k + 1));
            var twoRaisedToMinusK = Mathf.Pow(2, -k);
            var twoRaisedToMinus2K = Mathf.Pow(2, -2 * k);
            var twoRaisedToMinus2KMinus1 = Mathf.Pow(2, -2 * k - 1);
            ...
            for (var j = 1; j <= pCount; ++j)
            {
                ...
                var F = 0.5f - twoRaisedToMinusKPlus1 - (j - 1) * (twoRaisedToMinusK - j * twoRaisedToMinus2KMinus1);
                var G = 0.5f + twoRaisedToMinusKPlus1 + (j - 1) * (twoRaisedToMinusK - j * twoRaisedToMinus2K);
                var H = (j - 1) * j * twoRaisedToMinus2KMinus1;
