    ...
    public class ObiRopeCursor : MonoBehaviour
    {
        ...
        public void UpdateSource()
        {
            ...
            if (rope.isLoaded)
            {
                ...
                if (elm != null && rope.solver != null) m_SourceIndex = elmMu < 0.5f ? elm.particle1 : elm.particle2;
            }
        }
        ...
        private int AddParticleAt(int index)
        {
            ...
            var targetIndex = rope.activeParticleCount;
        ...
        public void ChangeLength(float newLength)
        {
            ...
            newLength = Mathf.Clamp(newLength, 0,
                (rope.sourceBlueprint.particleCount - 1) * rope.ropeBlueprint.interParticleDistance);

            // calculate the change in rope length:
            var lengthChange = newLength - rope.restLength;
            ...
            if (lengthChange < 0)
            {
                ...
                while (lengthChange > m_CursorElement.restLength)
                {
                    ...
                    var index = rope.elements.IndexOf(m_CursorElement);
            ...
            {
                var lengthDelta = Mathf.Min(lengthChange,
                    Mathf.Max(0, rope.ropeBlueprint.interParticleDistance - m_CursorElement.restLength));
                       ...
                       m_CursorElement.restLength + lengthChange > rope.ropeBlueprint.interParticleDistance)
                {
                    ...
                    {
                        // add new particle:
                        var newParticleSolverIndex =
                            AddParticleAt(solver.particleToActor[m_CursorElement.particle1].indexInActor);

                        // set position of the new particle:
                        solver.positions[newParticleSolverIndex] = solver.positions[m_CursorElement.particle1] +
                                                                   (solver.positions[m_CursorElement.particle2] -
                                                                    solver.positions[m_CursorElement.particle1]) *
                                                                   lengthDelta;
                    ...
                    else
                    {
                        // add new particle:
                        var newParticleSolverIndex =
                            AddParticleAt(solver.particleToActor[m_CursorElement.particle2].indexInActor);
