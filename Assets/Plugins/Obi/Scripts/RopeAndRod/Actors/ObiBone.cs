...
namespace Obi
{
    ...
    public class ObiBone : ObiActor, IStretchShearConstraintsUser, IBendTwistConstraintsUser, ISkinConstraintsUser
    {
        ...
        [Tooltip("Filter used for collision detection.")] [SerializeField]
        ...
        {
            var normalizedCoord = boneBlueprint.normalizedLengths[batch.particleIndices[constraintIndex * 2]];
        ...
        {
            var normalizedCoord = boneBlueprint.normalizedLengths[batch.particleIndices[constraintIndex]];
        ...
        private void FixRoot()
        {
            if (isLoaded)
            {
                ...
                solver.renderablePositions[rootIndex] =
                    solver.positions[rootIndex] = actor2Solver.MultiplyPoint3x4(Vector3.zero);

                // take particle rest orientation in actor space, and convert to solver space:
                solver.renderableOrientations[rootIndex] =
                    solver.orientations[rootIndex] = actor2SolverR * boneBlueprint.orientations[0];
            }
        }
        ...
        /// <summary>
        ///     Resets particle orientations/positions to match the current pose of the bone hierarchy, and sets all their
        ///     velocities to zero.
        /// </summary>
        public void ResetToCurrentShape()
        {
            ...
            for (var i = 0; i < particleCount; ++i)
            {
                ...
                var solverIndex = solverIndices[i];
                ...
                solver.renderablePositions[solverIndex] =
                    solver.positions[solverIndex] = world2Solver.MultiplyPoint3x4(trfm.position);

                var boneDeltaAWS = trfm.rotation * Quaternion.Inverse(boneBlueprint.restOrientations[i]);
                solver.renderableOrientations[solverIndex] = solver.orientations[solverIndex] = world2Solver.rotation *
                    boneDeltaAWS * boneBlueprint.root2WorldR * boneBlueprint.orientations[i];
            }
        }
        ...
        private void ResetReferenceOrientations()
        {
            if (boneBlueprint != null)
                for (var i = 1; i < boneBlueprint.restTransformOrientations.Count; ++i)
        ...
        private void UpdateRestShape()
        {
            ...
            var sbc =
                solver.GetConstraintsByType(Oni.ConstraintType.BendTwist) as
                    ObiConstraints<ObiBendTwistConstraintsBatch>;

            if (bendTwistConstraintsEnabled && bc != null && sbc != null)
                for (var j = 0; j < bc.GetBatchCount(); ++j)
                {
                    var batch = bc.GetBatch(j) as ObiBendTwistConstraintsBatch;
                    var solverBatch = sbc.batches[j];
                    var offset = solverBatchOffsets[(int) solverBatch.constraintType][j];

                    if (solverBatch.restDarbouxVectors.isCreated)
                        for (var i = 0; i < batch.activeConstraintCount; i++)
                        {
                            var indexA = batch.particleIndices[i * 2];
                            var indexB = batch.particleIndices[i * 2 + 1];

                            // calculate bone rotation delta in world space:
                            var boneDeltaAWS = boneBlueprint.transforms[indexA].rotation *
                                               Quaternion.Inverse(boneBlueprint.restOrientations[indexA]);
                            var boneDeltaBWS = boneBlueprint.transforms[indexB].rotation *
                                               Quaternion.Inverse(boneBlueprint.restOrientations[indexB]);

                            // apply delta to rest particle orientation:
                            var orientationA = boneDeltaAWS * boneBlueprint.root2WorldR *
                                               boneBlueprint.orientations[indexA];
                            var orientationB = boneDeltaBWS * boneBlueprint.root2WorldR *
                                               boneBlueprint.orientations[indexB];

                            solverBatch.restDarbouxVectors[offset + i] =
                                ObiUtils.RestDarboux(orientationA, orientationB);
                        }
                }
            ...
            if (skinConstraintsEnabled && sc != null && ssc != null)
                for (var j = 0; j < sc.GetBatchCount(); ++j)
                {
                    var batch = sc.GetBatch(j) as ObiSkinConstraintsBatch;
                    var solverBatch = ssc.batches[j];
                    var offset = solverBatchOffsets[(int) solverBatch.constraintType][j];

                    if (solverBatch.skinPoints.isCreated)
                        for (var i = 0; i < batch.activeConstraintCount; i++)
                        {
                            var index = batch.particleIndices[i];
                            solverBatch.skinPoints[offset + i] =
                                solver.transform.worldToLocalMatrix.MultiplyPoint3x4(boneBlueprint.transforms[index]
                                    .position);
                        }
                }
        }
        ...
        private void CopyParticleDataToTransforms()
        {
            if (boneBlueprint != null)
                ...
                for (var i = 1; i < particleCount; ++i)
                {
                    ...
                    var delta = GetParticleOrientation(solverIndices[i]) *
                                Quaternion.Inverse(boneBlueprint.root2WorldR * boneBlueprint.orientations[i]);
        ...
        public class BonePropertyCurve
        {
            ...
            public BonePropertyCurve(float multiplier, float curveValue)
            {
                ...
                curve = new AnimationCurve(new Keyframe(0, curveValue), new Keyframe(1, curveValue));
            }
