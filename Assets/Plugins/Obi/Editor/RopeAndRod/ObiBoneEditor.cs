...
namespace Obi
{
    ...
    public class ObiBoneEditor : Editor
    {
        ...
        private SerializedProperty fixRoot;
        ...
        private SerializedProperty rotationalMass;
        ...
        private SerializedProperty skinCompliance;

        private SerializedProperty skinConstraintsEnabled;
        private SerializedProperty skinRadius;
        private SerializedProperty stretchBones;
        ...
        public override void OnInspectorGUI()
        {
            ...
            var newCategory = EditorGUILayout.Popup("Collision category", ObiUtils.GetCategoryFromFilter(bone.Filter),
                ObiUtils.categoryNames);
            ...
            EditorGUI.BeginChangeCheck();
            var newMask = EditorGUILayout.MaskField("Collides with", ObiUtils.GetMaskFromFilter(bone.Filter),
                ObiUtils.categoryNames);
            if (EditorGUI.EndChangeCheck())
                foreach (ObiBone t in targets)
                {
            ...
            ObiEditorUtils.DoToggleablePropertyGroup(skinConstraintsEnabled,
                new GUIContent("Skin Constraints", Resources.Load<Texture2D>("Icons/ObiSkinConstraints Icon")),
                () =>
                {
        ...
        private static void DrawGizmos(ObiBone actor, GizmoType gizmoType)
        {
            if (actor.boneBlueprint != null && actor.isLoaded)
            {
                ...
                for (var i = 0; i < actor.boneBlueprint.parentIndices.Count; ++i)
                {
                    var parent = actor.boneBlueprint.parentIndices[i];
                    if (parent >= 0)
                    {
