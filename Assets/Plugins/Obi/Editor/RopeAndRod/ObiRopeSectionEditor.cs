...
namespace Obi
{
    ...
    {
        private readonly Color previewBck = new Color(0.2f, 0.2f, 0.2f, 1);
        private readonly Color previewLines = new Color(0.15f, 0.15f, 0.15f, 1);
        ...
        private void DrawSectionOutline(Rect region, Color color)
        {
            ...
            Handles.color = color;
            var points = new Vector3[section.vertices.Count];
            for (var i = 0; i < section.vertices.Count; i++)
        ...
        public override void OnInteractivePreviewGUI(Rect region, GUIStyle background)
        {
            ...
            {
                var x = region.center.x + section.vertices[i].x * region.width * 0.5f;
                var y = region.center.y + section.vertices[i].y * region.height * 0.5f;
                var pos = new Vector2(x, y);

                var oldSelection = selected[i];
                var olsPos = pos;
                ...
                {
                    pos.x = Mathf.Clamp(
                        ObiRopeSection.SnapTo(pos.x - region.center.x, section.snapX, 5) / (region.width * 0.5f), -1,
                        1);
                    pos.y = Mathf.Clamp(
                        ObiRopeSection.SnapTo(pos.y - region.center.y, section.snapY, 5) / (region.height * 0.5f), -1,
                        1);
