...
namespace Obi
{
    ...
    public class ObiStitcherEditor : Editor
    {
        ...
        public static Vector3 sewingToolHandle1 = Vector3.zero;
        public static Vector3 sewingToolHandle2 = Vector3.one;

        public static bool[] selectionStatus = new bool[0];

        private ObiStitcher stitcher;
        ...
        public void OnSceneGUI()
        {
            ...
            if (stitcher.Actor1 != null && stitcher.Actor2 != null)
            {
                var controlID = GUIUtility.GetControlID("stitcher".GetHashCode(), FocusType.Passive);
                var distanceToClosest = float.MaxValue;
                var selectedIndex = -1;
                var i = 0;

                foreach (var stitch in stitcher.Stitches)
                {
                    var pos1 = stitcher.Actor1.GetParticlePosition(
                        stitcher.Actor1.solverIndices[stitch.particleIndex1]);
                    var pos2 = stitcher.Actor2.GetParticlePosition(
                        stitcher.Actor2.solverIndices[stitch.particleIndex2]);
                    ...
                    switch (Event.current.GetTypeForControl(controlID))
                    {
                        case EventType.MouseDown:
                            ...
                            if ((Event.current.modifiers & EventModifiers.Shift) == 0 &&
                                ...
                                for (var j = 0; j < selectionStatus.Length; j++)
                                    selectionStatus[j] = false;

                            var distance = HandleUtility.DistanceToLine(pos1, pos2);
                ...
                if (selectedIndex >= 0) selectionStatus[selectedIndex] = !selectionStatus[selectedIndex];
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            var actor1 =
                EditorGUILayout.ObjectField("First actor", stitcher.Actor1, typeof(ObiActor), true) as ObiActor;
            ...
            EditorGUI.BeginChangeCheck();
            var actor2 =
                EditorGUILayout.ObjectField("Second actor", stitcher.Actor2, typeof(ObiActor), true) as ObiActor;
            if (EditorGUI.EndChangeCheck())
            {
            ...
            if (editing)
            {
                EditorGUILayout.HelpBox(
                    "Remember that when working with the sewing tool, you can use Unity's snap to vertex feature by pressing 'V' in your keyboard.",
                    MessageType.Info);

                // Clear all stitches
                if (GUILayout.Button("Clear all stitches"))
                    if (EditorUtility.DisplayDialog("Clearing stitches",
                            "Are you sure you want to remove all stitches?", "Ok", "Cancel"))
                    {
                ...
                {
                    var removedStitches = new List<int>();
                    ...
                    if (removedStitches.Count > 0)
                    {
                        ...
                        foreach (var i in removedStitches.OrderByDescending(i => i)) stitcher.RemoveStitch(i);
                    }
                }
        ...
        public void UseSewingTool(ref int particle1, ref int particle2)
        {
            ...
            if (stitcher.Actor1 == stitcher.Actor2)
            {
                ...
                for (var i = 0; i < stitcher.Actor1.particleCount; ++i)
                {
                    var pos = stitcher.Actor1.GetParticlePosition(stitcher.Actor1.solverIndices[i]);
                    var distance1 = (pos - sewingToolHandle1).sqrMagnitude;
                    var distance2 = (pos - sewingToolHandle2).sqrMagnitude;
            ...
            else
            {
                ...
                for (var i = 0; i < stitcher.Actor1.particleCount; ++i)
                {
                    ...
                    var distance1 = (pos - sewingToolHandle1).sqrMagnitude;
                    var distance2 = (pos - sewingToolHandle2).sqrMagnitude;
                    var min = Mathf.Min(distance1, distance2);
                    if (min < minDistance)
                    {
                        minDistance = min;
                        particle1 = i;
                ...
                for (var i = 0; i < stitcher.Actor2.particleCount; ++i)
                {
                    var pos = stitcher.Actor2.GetParticlePosition(stitcher.Actor2.solverIndices[i]);
                    var distance1 = (pos - sewingToolHandle1).sqrMagnitude;
                    var distance2 = (pos - sewingToolHandle2).sqrMagnitude;
                    var min = Mathf.Min(distance1, distance2);
                    if (min < minDistance)
        ...
        {
            sewingToolHandle1 = Handles.FreeMoveHandle(sewingToolHandle1, Quaternion.identity,
                HandleUtility.GetHandleSize(sewingToolHandle1) * 0.05f, new Vector3(.5f, .5f, .5f),
                Handles.RectangleHandleCap);
            sewingToolHandle2 = Handles.FreeMoveHandle(sewingToolHandle2, Quaternion.identity,
                HandleUtility.GetHandleSize(sewingToolHandle2) * 0.05f, new Vector3(.5f, .5f, .5f),
                Handles.RectangleHandleCap);
