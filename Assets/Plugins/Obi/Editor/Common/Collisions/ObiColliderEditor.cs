...
namespace Obi
{
    [CustomEditor(typeof(ObiColliderBase), true)]
    [CanEditMultipleObjects]
    public class ObiColliderEditor : Editor
    {
        private ObiColliderBase collider;
        private SerializedProperty collisionFilter;
        ...
        public override void OnInspectorGUI()
        {
            ...
            var newCategory = EditorGUI.Popup(rect, ObiUtils.GetCategoryFromFilter(collider.Filter),
            ...
            EditorGUI.BeginChangeCheck();
            var newMask =
                EditorGUI.MaskField(rect, ObiUtils.GetMaskFromFilter(collider.Filter), ObiUtils.categoryNames);
            if (EditorGUI.EndChangeCheck())
                foreach (ObiColliderBase t in targets)
                {
