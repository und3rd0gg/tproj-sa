...
namespace Obi
{
    ...
    public class ObiActorBlueprintEditor : Editor, IObiSelectableParticleProvider
    {
        ...
        private BooleanPreference showRenderModes;
        protected float[] sqrDistanceToCamera = new float[0];

        public ObiActorBlueprint blueprint => target as ObiActorBlueprint;

        public ObiBlueprintPropertyBase currentProperty => GetProperty(currentPropertyIndex);

        public ObiBlueprintEditorTool currentTool => GetTool(currentToolIndex);
...
#if (UNITY_2019_1_OR_NEWER)
            renderCallback = (cntxt, cam) => { DrawWithCamera(cam); };
        ...
        public virtual void OnDisable()
        {
            ...
            foreach (var renderMode in renderModes) renderMode.OnDestroy();
        ...
        public virtual void OnSceneGUI(SceneView sceneView)
        {
            ...
            if (e.type == EventType.Repaint)
            {
                ...
                ObiParticleEditorDrawing.DrawParticles(sceneView.camera, blueprint, activeParticle, visible, tint,
                    sortedIndices, dotRadiusScale);
            }
        ...
        protected void Generate()
        {
            ...
            {
                if (EditorUtility.DisplayDialog("Blueprint generation",
                        "This blueprint already contains data. Are you sure you want to re-generate this blueprint from scratch?",
                        "Ok", "Cancel"))
                {
                    EditorUtility.SetDirty(target);
                    var job = new CoroutineJob();
        ...
        protected virtual bool ValidateBlueprint()
        {
            return true;
        }
        ...
        {
            var regularColor = ObiEditorSettings.GetOrCreateSettings().particleColor;
            var selectedColor = ObiEditorSettings.GetOrCreateSettings().selectedParticleColor;
            var activeColor = ObiEditorSettings.GetOrCreateSettings().activeParticleColor;

            for (var i = 0; i < blueprint.positions.Length; i++)
            {
                // get particle color:
                if (activeParticle == i)
                    tint[i] = activeColor;
        ...
        public virtual void UpdateParticleVisibility()
        {
            for (var i = 0; i < blueprint.positions.Length; i++)
                if (blueprint.IsParticleActive(i))
                {
                    ...
                    {
                        var camToParticle = Camera.current.transform.position - blueprint.positions[i];
        ...
        {
            var contents = new GUIContent[tools.Count];
            ...
            EditorGUILayout.BeginVertical(EditorStyles.inspectorDefaultMargins);
            EditorGUI.BeginChangeCheck();
            var newSelectedTool = ObiEditorUtils.DoToolBar(currentToolIndex, contents);
            EditorGUILayout.EndVertical();
            ...
            if (currentTool != null)
            {
                ...
                var help = currentTool.GetHelpString();
