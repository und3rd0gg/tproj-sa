...
namespace Obi
{
    public abstract class ObiBlueprintPropertyBase
    {
        ...
        public virtual Color ToColor(int index)
        {
            return Color.white;
        }
        ...
        public void BrushModes(ObiBrushBase paintBrush)
        {
            ...
            var contents = new GUIContent[brushModes.Count];
            for (var i = 0; i < brushModes.Count; ++i)
                contents[i] = new GUIContent(brushModes[i].name);

            EditorGUI.BeginChangeCheck();
            selectedBrushMode = ObiEditorUtils.DoToolBar(selectedBrushMode, contents);
            if (EditorGUI.EndChangeCheck()) paintBrush.brushMode = brushModes[selectedBrushMode];
        }
    }
    ...
    public abstract class ObiBlueprintProperty<T> : ObiBlueprintPropertyBase
    {
        ...
        public override void GetDefaultFromIndex(int index)
        {
            value = Get(index);
        }

        public override void SetDefaultToIndex(int index)
        {
            Set(index, value);
        }
