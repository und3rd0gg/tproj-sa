...
namespace Obi
{
    public class ObiPropertyTextureEditorTool : ObiBlueprintEditorTool
    {
        ...
        protected float minPropertyValue;
        ...
        private void FloatFromTexture(int i, Color color)
        {
            ...
            {
                var value = minPropertyValue + color[(int) textureChannel] * (maxPropertyValue - minPropertyValue);
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                propertyTexture =
                    (Texture2D) EditorGUILayout.ObjectField("Source", propertyTexture, typeof(Texture2D), false);
                ...
                if (GUILayout.Button("Import"))
                {
                    ...
                    if (!meshBasedEditor.ReadParticlePropertyFromTexture(propertyTexture, textureReadCallback))
                        EditorUtility.DisplayDialog("Invalid texture", "The texture is either null or not readable.",
                            "Ok");
                ...
                if (GUILayout.Button("Export"))
                {
                    ...
                    if (path.Length > 0)
                    {
                        ...
                        if (!meshBasedEditor.WriteParticlePropertyToTexture(path, exportWidth, exportHeight, padding))
                            EditorUtility.DisplayDialog("Invalid path", "Could not write a texture to that location.",
                                "Ok");
                    }
                }
            }
