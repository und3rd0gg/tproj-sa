...
namespace Obi
{
    public class ObiTethersTool
    {
        ...
        private void OnTetherGroupSelected(object index)
        {
            var i = (int) index;
        ...
        public void DoTethers(ObiActorBlueprintEditor editor)
        {
            ...
            if (tetherCount > 0)
                ...
                EditorGUILayout.LabelField(
                    "No tether constraints. Select at least one particle group in the dropdown, then click 'Generate Tethers'.",
            ...
            if (GUILayout.Button("Tethered groups", EditorStyles.popup))
            {
                ...
                for (var i = 0; i < editor.blueprint.groups.Count; ++i)
                    menu.AddItem(new GUIContent(editor.blueprint.groups[i].name), tetheredGroups[i],
                        OnTetherGroupSelected, i);
            ...
            if (GUILayout.Button("Generate tethers", GUILayout.MinHeight(32)))
            {
                ...
                for (var i = 0; i < editor.selectionStatus.Length; ++i)
                {
                    editor.selectionStatus[i] = false;
                    for (var j = 0; j < tetheredGroups.Length; ++j)
