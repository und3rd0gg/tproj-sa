...
namespace Obi
{
    public class ObiParticleSelectionEditorTool : ObiBlueprintEditorTool
    {
        ...
        private void InitializeGroupsList()
        {
            ...
            particleGroupList.drawHeaderCallback = (Rect rect) => { EditorGUI.LabelField(rect, "Groups"); };
            ...
            particleGroupList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                if (GUI.Button(new Rect(rect.x, rect.y, rect.width * 0.5f, EditorGUIUtility.singleLineHeight), "Select",
                ...
                if (GUI.Button(
                        new Rect(rect.x + rect.width * 0.5f, rect.y, rect.width * 0.5f,
                            EditorGUIUtility.singleLineHeight), "Set", EditorStyles.miniButtonRight))
                {
        ...
        private void SelectionTools()
        {
            ...
            if (GUILayout.Button(new GUIContent(Resources.Load<Texture2D>("InvertButton"), "Invert selection"),
                    GUILayout.MaxHeight(24), GUILayout.MaxWidth(48)))
            {
                for (int i = 0; i < editor.selectionStatus.Length; i++)
                {
            ...
            if (GUILayout.Button(new GUIContent(Resources.Load<Texture2D>("ClearButton"), "Clear selection"),
                    GUILayout.MaxHeight(24), GUILayout.MaxWidth(48)))
            {
                for (int i = 0; i < editor.selectionStatus.Length; i++)
            ...
            if (GUILayout.Button(new GUIContent(Resources.Load<Texture2D>("OptimizeButton"), "Optimize selected"),
                    GUILayout.MaxHeight(24), GUILayout.MaxWidth(48)))
            {
            ...
            if (GUILayout.Button(new GUIContent(Resources.Load<Texture2D>("RemoveButton"), "Remove selected"),
                    GUILayout.MaxHeight(24), GUILayout.MaxWidth(48)))
            {
            ...
            if (GUILayout.Button(
                    new GUIContent(Resources.Load<Texture2D>("RestoreButton"), "Restore removed particles"),
                    GUILayout.MaxHeight(24), GUILayout.MaxWidth(48)))
            {
            ...
            if (property != null)
            {
                if (!Mathf.Approximately(property.minVisualizationValue, property.maxVisualizationValue))
                {
                    EditorGUILayout.HelpBox(
                        "Drag the slider to select based on " + property.name +
                        ". You can choose a different property in the \"Property\" dropdown below.", MessageType.None);
                    ...
                    EditorGUILayout.MinMaxSlider("Select by " + property.name, ref minSelectionValue,
                        ref maxSelectionValue, property.minVisualizationValue, property.maxVisualizationValue);
                ...
                {
                    EditorGUILayout.HelpBox("All particles have the same " + property.name + " value.",
                        MessageType.Info);
                }
            }
            else
            {
                EditorGUILayout.HelpBox("Property-based selection only works with scalar properties.",
                    MessageType.Info);
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUI.BeginChangeCheck();
                (editor as ObiMeshBasedActorBlueprintEditor).particleCulling =
                    (ObiMeshBasedActorBlueprintEditor.ParticleCulling) EditorGUILayout.EnumPopup("Culling",
                        (editor as ObiMeshBasedActorBlueprintEditor).particleCulling);
                if (EditorGUI.EndChangeCheck())
            ...
            EditorGUILayout.HelpBox(
                "Select a property to view and edit. Currently editing " + editor.currentProperty.name + ".",
                MessageType.None);
            ...
            editor.dotRadiusScale =
                EditorGUILayout.Slider(new GUIContent("Particle dot size"), editor.dotRadiusScale, 0, 5);
