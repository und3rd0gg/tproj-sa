...
void BuildParameterSpaceMatrices(float4 t0, float4 t1, float4 t2, out float3x3 P, out float3x3 IP)
{
    ...
    float3x3 IS = float3x3(1 / (_RadiusScale * t0.w), 0, 0, 0, 1 / (_RadiusScale * t1.w), 0, 0, 0,
                           1 / (_RadiusScale * t2.w));
...
float BuildEllipsoidBillboard(float3 center, float3 corner, float3x3 P, float3x3 IP, out float3 worldPos,
                              out float3 view, out float3 eye)
{
...
void BuildAuxiliaryNormalVectors(float3 center, float3 worldPos, float3 view, float3x3 P, float3x3 IP, out float3 a2,
                                 out float3 a3)
{
